...
// GetConfigChangeNotifier will return a ConfigWatcher if either the configFile or configOverlay are populated.
// If neither config is populated an empty config watcher is returned which is entirely valid to use,
// but will never return events.
func (ib *IngesterBase) GetConfigChangeNotifier() (cw ConfigWatcher, err error) {
	if ib.configFile != `` || ib.configOverlay != `` {
		cw, err = newConfigWatcher(ib.configFile, ib.configOverlay)
	}
	return
}
